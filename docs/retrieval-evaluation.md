...
- 回答可能 49 問
- 回答不能 11 問
- 17 份の金融 Markdown をすべてカバー
- KYC、口座、振込、AML、融資、信用リスク、個人情報、苦情、サイバー、BCP、適合性、規制変更を含む
...
この 100% は、リポジトリ内で作成した固定評価セットに対する検索結果です。実利用者の質問、誤字、曖昧表現、新しい商品・規制を含む一般的な正答率を意味しません。次段階では、別担当者が作成した未見質問と実利用ログを追加し、データ漏洩を避けた回帰評価を行います。
